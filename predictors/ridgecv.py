...
try:
    ...
    SKLEARN_OK = False
    print("[WARN] sklearn не установлен, RidgeCVPredictor будет использовать случайные прогнозы")


class RidgeCVPredictor(BasePredictor):
    def __init__(self, total_numbers: int, pick_count: int, alphas: List[float] = None, name: str = "RidgeCV"):
        # Исправлено: передаём total_numbers и pick_count
        super().__init__(total_numbers, pick_count)
        self.total_numbers = total_numbers
        self.pick_count = pick_count
        self.alphas = alphas if alphas else [0.1, 1.0, 10.0]
        self.model = None
        self.last_blocks = []
