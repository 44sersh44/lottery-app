...
class ExtraTreesPredictor(BasePredictor):
    def __init__(self, total_numbers: int, pick_count: int, n_estimators: int = 100, name: str = "ExtraTrees"):
    ...
    def fit(self, blocks: List[List[int]]) -> None:
        ...
        self.model = ExtraTreesRegressor(n_estimators=self.n_estimators, random_state=42)
        self.model.fit(X, y)
        self.is_trained = True
