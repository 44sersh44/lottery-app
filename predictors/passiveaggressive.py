...
class PassiveAggressivePredictor(BasePredictor):
    def __init__(self, total_numbers: int, pick_count: int, C: float = 1.0, name: str = "PassiveAggressive"):
    ...
    def fit(self, blocks: List[List[int]]) -> None:
        ...
        self.model = PassiveAggressiveRegressor(C=self.C)
        self.model.fit(X, y)
        self.is_trained = True
